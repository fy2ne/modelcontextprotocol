...
Servers **MAY** emit `notifications/progress` notifications during a `tools/call` request to stream intermediate results. Each notification can include an optional `contentChunk` field carrying partial tool output.

For ordering, each chunk includes a `seq` field — a monotonic integer scoped to the request's `progressToken`.

**Streaming flow:**

```jsonc
// Server sends intermediate chunks during tool execution
{
  "jsonrpc": "2.0",
...
}
{
  ...
  "params": {
    ...
    "contentChunk": {
      "type": "text",
      "text": "Processing complete. Generating summary..."
    }
  }
}
...
// Final authoritative result
{
  ...
  "result": {
    "resultType": "complete",
    "content": [
      { "type": "text", "text": "Found 3 documents. Summary: ..." }
    ],
    "isError": false
  }
}
...
The final `tools/call` response is always the authoritative result. Clients that do not handle `contentChunk` simply ignore the optional field.
...
sequenceDiagram
    ...
    Note over Client,Server: Invocation with streaming
    Client->>Server: tools/call
    Server--)Client: notifications/progress (seq: 1, contentChunk)
    Server--)Client: notifications/progress (seq: 2, contentChunk)
    Server--)Client: notifications/progress (seq: 3, contentChunk)
    Server-->>Client: tools/call result (authoritative)
```
